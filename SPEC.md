...
- **GA列は色分け対象外**(期限ではなく開始日のため)。オプトイン設定で
  「リリースから○日」のグレーバッジ(色なし)を表示可能(デフォルトOFF)。
  期限系バッジとの誤読を避けるため文言を分けている
...
- バッジ / 凡例 / 取り消し線 / GA経過日数バッジ(デフォルトOFF)の ON/OFF
...
- **期限の抽出は2段構え**: セル内に `<pfe-datetime datetime="...">`(OpenShift /
  全製品ページなど)があれば `.end-date` コンテナ内の datetime 属性(ISO、言語非依存)を使う。
  期間の終了側が `Ongoing`(datetime なし)のセルはスキップ。
  なければテキストをパース(Operators ページなど)。対応形式:
...
scripts/check-structure.mjs  構造検証のエントリポイント
...
| `release.yml` | **手動 (`workflow_dispatch`)** / `v*` タグ push | 手動時は `patch`/`minor`/`major` で version を用意→テスト/ビルド成功後に main へ commit&tag→Chrome Web Store へアップロード&公開→GitHub Release 作成。失敗後の再実行は「main がストアより先」ならバンプせず再開。タグ push 時はバンプなしで同じ公開処理 |
...
表の検知条件: ヘッダーに **General availability**(日本語表示では 一般提供)列を
含むこと。それ以外の列構成は問わない——どの列を色分けするかは列名ではなく
「セルから期限(日付)が取れるか」のデータ駆動で決まる。これにより
Full support 型(OCP/RHEL)、End of Life 型(.NET)、Maintenance Support 1/2 型
(Ansible Core)など製品ごとの列構成の違いを吸収する。列名の知識が必要なのは
GA / Version の除外判定のみ。

## スコープ外(将来候補)

- GA列を持たないライフサイクル表への対応
